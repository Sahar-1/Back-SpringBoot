...
class EspritPiDevApplicationTests {
	...
	/*@Test
	public void testAdd1() throws ParseException, java.text.ParseException {
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		Date d = dateFormat.parse("1995-03-05");
		...
		System.out.println("********  testAdd  addRole*************");
		Dbo_Role dbo_Role=	ur.addRole(role);
		System.out.println("********  testAdd1*************");
		Dbo_User user = new Dbo_User();
		user.setFirstName("Ayoub");
		user.setLastName("Ben zahra");
		user.setEmail("ayoub.benzahra@esprit.tn");
		user.setDate(d);
